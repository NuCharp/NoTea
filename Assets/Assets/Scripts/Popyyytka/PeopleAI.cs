...
public class CubeAI : MonoBehaviour
{
    ...
    public TrafficLightCone pedestrianLight;  // Ссылка на светофор
    ...
    // Метод для определения, когда куб решает отдохнуть (например, 20% шанс)
    bool ShouldRest()
    {
    ...
    // Взаимодействие с триггером для пешеходного перехода
    void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("PedestrianTrigger") && pedestrianLight != null)
        {
            if (!pedestrianLight.IsGreenLight())
            {
                agent.isStopped = true;
                Debug.Log("Куб остановился на пешеходном переходе.");
            }
        }
    }
    ...
    void OnTriggerStay(Collider other)
    {
        if (other.CompareTag("PedestrianTrigger") && pedestrianLight != null)
        {
            if (pedestrianLight.IsGreenLight())
            {
                ...
                Debug.Log("Куб продолжил движение на зелёный свет.");
            }
        }
    }
}
